...
function Book(name, author, year, description, image, read = false) {
...
Book.prototype.toggleRead = function(e) {
  //marcar o no como leido
  if (this.read) e.target.textContent = "Read";
...
window.addEventListener("load", () => {
  myLibrary = localStorage.getItem("array");
  if (myLibrary) {
    ...
    let Book1 = new Book(
      ...
      "https://contentv2.tap-commerce.com/cover/large/9789875669284_1.jpg?id_com=1113"
    );
    let Book2 = new Book(
      "Brave New World",
      "Aldous Huxley",
      ...
      "https://http2.mlstatic.com/book-brave-new-world-aldous-huxley-D_NQ_NP_819086-MLA27844935653_072018-F.jpg"
    );
    myLibrary = [];
...
function addBookToLibrary() {
  //función temporal para añadir libros.
...
function render(elem, index) {
  //creacion de los elementos HTML pertenecientes a cada libro
  if (elem) {
    ...
    } else markRead.textContent = "Read";
...
function setForm() {
  //establece el form con los datos del libro
...
function addition(formBook) {
  ...
  if (bookIndex == -1) bookIndex = myLibrary.length;
  myLibrary[bookIndex] = formBook;
  render(myLibrary[bookIndex], bookIndex);
}

function edition(formBook) {
  //editar el html.
  edit.node
    .getElementsByClassName("cover")[0]
    .setAttribute("src", formBook.image);
  edit.node.getElementsByClassName("title")[0].childNodes[0].textContent =
    formBook.name;
  ...
  edit.node.getElementsByClassName("description")[0].textContent =
